...
class LSTM_Gen(object):
    def __init__(self, alpha_c=0.0, dropout=True):
        ...
        self.l_pos_w = 0.15  # Weight of the POS loss function, 0.3
        self.null_penalty = 4
        self.tag_penalty = 50
        ...
        self.pos_gt = tf.placeholder(tf.int32, shape=[None, 22, 12])
        self.sequence_id = tf.placeholder(tf.int32, shape=[None, 1])

        self.idx_sep = tf.placeholder(tf.int64, shape=[])
    ...
    def _decode_lstm_pos_1(self, h, context, scope='logits'):
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            w_h = tf.get_variable('w_h', [self.H, 22*self.M], initializer=self.weight_initializer)
            b_h = tf.get_variable('b_h', [22*self.M], initializer=self.const_initializer)
            w_out = tf.get_variable('w_out', [22*self.M, 22*12], initializer=self.weight_initializer)
            b_out = tf.get_variable('b_out', [22*12], initializer=self.const_initializer)
            ...
            if self.USE_DROPOUT:
                h_logits = tf.nn.dropout(h_logits, 0.5)
            out_logits = tf.matmul(h_logits, w_out) + b_out
            out_logits = tf.reshape(out_logits, [-1, 22, 12])

            return out_logits

    def _decode_lstm_pos_2(self, h, context, logits_pos, scope='logits2'):
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            w_h = tf.get_variable('w_h', [self.H, 22*self.M], initializer=self.weight_initializer)
            b_h = tf.get_variable('b_h', [22*self.M], initializer=self.const_initializer)
            w_out = tf.get_variable('w_out', [22*self.M, 22*config.dict_size], initializer=self.weight_initializer)
            ...
            logits_pos = tf.reshape(logits_pos, [-1, 22 * 12])
            ...
            w_pos2out = tf.get_variable(
                name='w_pos2out',
                shape=[22 * 12, 22 * self.M],
                initializer=self.weight_initializer
            )
            h_logits += tf.matmul(logits_pos, w_pos2out)
    ...
    def build_model(self):
        ...
        logits_pos = self._decode_lstm_pos_1(h, z, scope='logits_acc')

        logits_caption = self._decode_lstm_pos_2(h, z, logits_pos, scope='logits_2')

        loss = (1-self.l_pos_w) * tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=caption_onehot, logits=logits_caption))
        loss += self.l_pos_w * tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=pos_gt_onehot, logits=logits_pos))

        # Null Tag penalty
        # logits_cap_softmax = tf.nn.softmax(logits_caption)
        logits_cap_max = tf.math.argmax(input=logits_caption, axis=2)
        loss += tf.cast(tf_count(logits_cap_max, 0) * self.null_penalty, dtype=tf.float32)

        # Multiple Tag Penalty (SEP, START, END)
        for i in range(batch_size):
            loss_tags = tf_is_once(logits_cap_max[i], 1) * self.tag_penalty + tf_is_once(logits_cap_max[i],2) * self.tag_penalty + tf_is_once(logits_cap_max[i], idx_sep) * self.tag_penalty
            loss += tf.cast(loss_tags, dtype=tf.float32)
