...
class PreProcessor_CNN_4frame():
    def __init__(self):
        ...
        with tf.variable_scope("pre-processor"):
            ...
            self.inputImg = tf.placeholder(shape=[None, config.timelen, config.imgRow, config.imgCol, config.imgCh], dtype=tf.float32) #float32
            ...
            dst = tf.reshape(dst, [config.batch_size, config.imgRow, config.imgCol, -1]) # array [batch_size, imgRow_resized, imgCol_resized, num_frames*channels]
