    ...
    def __init__(self, timelen=config.timelen, phase='train'):
        ...
        with tf.variable_scope("pre-processor"):
            ...
            if phase == 'train':
                ...
                # get info value at frame-series for last frame of series (#4) (i.e. shift by 3)
                ...
                # get 20 feature cubes per batch sample with 20 log values

            # feats
            dst = tf.reshape(   inputImg, [config.batch_size, timelen-START_FRAME, 64, 12*20] )
            dst = tf.reshape(   dst, [-1,64,12*20] ) # Returns 20*BS feature cubes
            ...
            # Values are scaled, so per sample 20 feature cubes and 20 scaled log values
