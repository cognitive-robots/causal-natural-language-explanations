...
class VA(object):
  ...
  def _get_initial_lstm(self, features):
    with tf.variable_scope('initial_lstm'):
      features_mean = tf.reduce_mean(features, 1) # calc mean along spatial locations
  ...
  def _attention_layer(self, features, features_proj, h, reuse=False):
    ...
    with tf.variable_scope('attention_layer', reuse=reuse):
      ...
      #print((features * tf.expand_dims(alpha, 2)).shape)
  ...
  def build_model(self):
    ...
    features_init        = tf.gather( features, gather_indices_init ) # gather along each feature cube of max BS*20
    ...
    loss_course = 0.0
    alpha_reg   = 0.0
    for t in range(self.T+1): #range 20
      gather_indices     = tf.range(config.batch_size) * self.timelen + t # -> After the for loop: BS*20 samples have been processed (because they are saved in chunks of 20 per sample)
      ...
      loss  += tf.reduce_sum( tf.abs( tf.subtract( logits_acc,    y_acc_curr    ) ) ) # reduce_sum => Sum over Batch Size, loss+= sum over 20 cubes per batch sample
      loss  += tf.reduce_sum( tf.abs( tf.subtract( logits_course, y_course_curr ) ) ) # here loss for sample amount = batch size (need to calculate: /BS)
      loss_acc += tf.reduce_sum( tf.abs( tf.subtract( logits_acc,    y_acc_curr    ) ) ) # Need to scale back with 100
      loss_course += tf.reduce_sum( tf.abs( tf.subtract( logits_course, y_course_curr ) ) ) # Need to scale back with 10

    loss += alpha_reg # here loss for all samples of timelen 20 (need to calc: /20)
    ...
    first_sum_acc = tf.summary.scalar(name='Validation loss Acc', tensor=loss_acc)
    first_sum_course = tf.summary.scalar(name='Validation loss Course', tensor=loss_course)

    return loss, alpha_reg, first_summary_train, first_summary_val, first_sum_acc, first_sum_course
  ...
  def inference(self):
    ...
    y_acc_list, y_course_list, alpha_list, context_list = [], [], [], []
    mae_accel = 0.0
    mae_course = 0.0
    for t in range(self.T+1):
      ...
      mae_accel += tf.reduce_sum(tf.abs(tf.subtract(logits_acc, y_acc_curr))) / (config.batch_size) # reduce_sum => Sum over Batch Size, loss+= sum over 20 cubes per batch sample
      mae_course += tf.reduce_sum(tf.abs(tf.subtract(logits_course, y_course_curr))) / (config.batch_size)

      # accumulation
      y_acc_list.append(logits_acc)
      y_course_list.append(logits_course)
      alpha_list.append(alpha)
      context_list.append(context_raw)

    mae_accel = mae_accel / (self.T+1)
    mae_course = mae_course / (self.T+1)

    alphas    = tf.transpose(tf.stack(values=alpha_list), (1,0,2))
    ys_acc    = tf.squeeze(y_acc_list)
    ys_course = tf.squeeze(y_course_list)
    contexts = tf.transpose(tf.stack(values=context_list), (1,0,2))

    return alphas, contexts, ys_acc, ys_course, mae_accel, mae_course

  def inference_SAX(self):
    features = self.features
    y_acc = self.target_acc
    y_course = self.target_course
    speed    = self.speed
    ...
    # Initialize LSTM
    gather_indices_init  = tf.range(config.batch_size) * 500
    features_init        = tf.gather( features, gather_indices_init )
    c, h                 = self._get_initial_lstm(features=features_init)
    lstm_cell            = tf.nn.rnn_cell.BasicLSTMCell(num_units=self.H)
    ...
    mae_course = 0.0
    for t in range(500):
      gather_indices     = tf.range(config.batch_size) * 500 + t
      features_curr      = tf.gather( features, gather_indices )
      features_proj_curr = tf.gather( features_proj, gather_indices )
      speed_curr         = tf.gather( speed, gather_indices )
      goaldir_curr       = tf.gather( goaldir, gather_indices )
    ...
    mae_accel = mae_accel / (500)
    mae_course = mae_course / (500)
